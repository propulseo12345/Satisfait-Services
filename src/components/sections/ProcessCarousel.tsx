...
const colorMap: Record<string, { color: string; iconBg: string; iconColor: string }> = {
  blue: { color: "from-blue-500 to-blue-600", iconBg: "bg-blue-100", iconColor: "text-blue-600" },
  purple: { color: "from-violet-500 to-purple-600", iconBg: "bg-violet-100", iconColor: "text-violet-600" },
  green: { color: "from-emerald-500 to-teal-600", iconBg: "bg-emerald-100", iconColor: "text-emerald-600" },
  orange: { color: "from-orange-500 to-amber-600", iconBg: "bg-orange-100", iconColor: "text-orange-600" },
};

function ProcessCarousel({ data }: ProcessCarouselProps) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, amount: 0.2 });

  const badge = data?.badge || "Simple et efficace";
  const mainTitle = data?.mainTitle || "Comment ça";
  const highlightedText = data?.highlightedText || "fonctionne ?";
  const subtitle = data?.subtitle || "4 étapes simples pour un service de qualité";

  const steps: Step[] = data?.steps?.map((s) => {
    const colors = colorMap[s.bgColor] || colorMap.blue;
    return {
      number: s.number,
      title: s.title,
      description: s.description,
      icon: iconMap[s.icon] || Phone,
      color: colors.color,
