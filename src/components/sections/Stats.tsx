...
// Mapping des couleurs par icône
const colorMap: Record<string, { color: string; bgColor: string }> = {
  star: { color: "text-yellow-500", bgColor: "bg-yellow-50" },
  users: { color: "text-blue-500", bgColor: "bg-blue-50" },
  clock: { color: "text-green-500", bgColor: "bg-green-50" },
  mapPin: { color: "text-purple-500", bgColor: "bg-purple-50" },
};
...
function Stats({ data }: StatsProps) {
  ...
  // Valeurs par défaut
  const sectionTitle = data?.sectionTitle || "Satisfait Services en";
  const sectionHighlight = data?.sectionHighlight || "quelques chiffres";
  const subtitle = data?.subtitle || "Des résultats qui témoignent de notre engagement";

  // Parser la value string pour extraire nombre et suffixe
  const parseValue = (valueStr: string): { value: number; suffix: string } => {
  ...
  // Transformer les données Sanity en format Stat avec couleurs
  const stats: Stat[] = data?.stats?.map((statData) => {
    const { value, suffix } = parseValue(statData.value);
    const Icon = iconMap[statData.icon] || Star;
    const colors = colorMap[statData.icon] || colorMap.star;
